...
1. As profissões principais (que precisarão de setagem) do servidor serão: as Forças Policiais, SAMU, Táxi, Mecânico, Facções, Imprensa;
    ...
    1. As profissões legais devem agir de forma justa e honesta para o bom funcionamento da cidade e do RP;
...
1. É obrigatório porte de arma para civis portarem armas de baixo calibre utilizadas para proteção pessoal (five seven), cabendo o uso ostensivo apenas para as forças policiais em serviço. Qualquer outra situação em que um civil estiver portando uma arma estará de forma ilegal.

### ORGANIZAÇÃO POLICIAL

1. Sobre as Forças Policias, o ingresso será realizado mediante avaliações disponibilizados pela própria corporação;

    1. Todas as atividades policiais deverão ser realizadas conforme regras operacionais de cada subdivisão;

1. Sobre as Forças Policiais, todos aqueles que ingressarem na corporação deverão ter em mente que estão representando uma corporação, prezando a camaradagem, respeito e empatia seja com os colegas quanto com os cidadãos;

    1. Todo território está sob os cuidados da Organização Policial Unificada que possui 3 subdivisões: Polícia de Patrulhamento, Polícia Ostensiva e Polícia Investigativa;
    ...
    1. É Proibido atuação a paisana, salvo em roleplay para alguma investigação em procedimento;
    ...
    1. Caberá a Polícia de Patrulhamento e a Polícia Ostensiva a fiscalização direta e obrigatoriedade de fornecer segurança dentro da cidade;
    ...
    1. Quando realizado alguma apreensão, ou prisão, deverá ser encaminhado o infrator, os policiais e todas as demais provas para o Comandante da Polícia envolvida e realizar o devido registro do que foi realizado do Registro Policial;
...
1. Sobre perseguições, ações e interações policiais, deverão ser seguidas as regras para um bom roleplay, seja pelo lado policial quanto pelo lado não policial;
    ...
    1. Em perseguição às forças policiais só poderão atirar contra o veículo em fuga (Código 5), quando esgotado as formas de tentativa de parada do mesmo;
    ...
    1. Em perseguição, é extremamente proibido fuga para locais de Safe-zone;

    1. A polícia não pode utilizar informações no @anônimo ou #Deepweb;

    1. Após perseguição, enquanto foragido, o mesmo deverá ficar pelo tempo de 5 minutos (Tempo de Nárnia) com a mesma aparência, veículo e atributos.
...
1. É responsabilidade do SAMU prestar por atendimento médico e emergencial para todos os cidadãos da cidade, independente de quem seja, todos deverão ser tratados com respeito e dedicação;
...
1. Os preços dos atendimentos do Mecânico serão taxados pela própria administração da corporação, entretanto, será necessário aprovação do Prefeito da cidade para que os valores estabelecidos sejam aprovados;
...
1. Toda  negociação entre facções por serviços como lavagem de dinheiro, tráfico, ou qualquer outra ação deve ser feita usando bom senso par que não ocorra desbalanceamento das ações policiais;
...
1. É proibido a realização de ações nas imediações da base Policial;

1. O Crime organizado é responsável por todas as favelas e comunidades do mapa (no caso, 3 favelas); Caso outras facções sejam criadas, iniciaremos o RP de invasão de favelas, onde a nova facção poderá se organizar para tomar uma favela do crime organizado.

1. Milícia: Visto que a milícia é composta por policiais e ex-policiais, para fazer parte da Facção milícia na cidade, o jogador precisará antes ter passado pelo menos duas semanas (tempo de nárnia) como policial. Após esse período ele poderá entrar para a milícia seguindo as regras da facção, sendo um policial ainda, ou não. Caso seja descoberto, sofrerá penas dentro do RP como exoneração e prisão. A milícia não poderá ter membros que não atendem esses requisitos.

### RESGATE DE PRISIONEIROS

1. O portão prisão é o local limite de resgate. Caso esteja acontecendo uma ação de resgate e cheguem nesse ponto, os envolvidos devem abandonar a tentativa e desistir.

1. Resgates deverão ser realizados apenas no trajeto do Departamento de Polícia até a Penitenciária;

1. É proibido realização de resgate dentro da base policial;
...
1. Todas deverão ser feitas de forma ordenada para a diversão de todos.

1. Todas as ações devem ser supervisionadas por um moderador/oráculo.
...
1. Quando em ações policiais, deverá ser respeitado a jurisdição da polícia que está respondendo o chamado;
...
1. Armas pesadas não serão divididas entre ações mas sim entre quantidade de membros em uma ação, no caso, mínimo 4 players para cada lado. Por exemplo: Se em um sequestro temos 4 policias de um lado e 4 bandidos de outro, os fuzis estão liberados, porém, se em um sequestro temos 2 policiais de um lado e 2 bandidos de outro, somente pistolas estão permitidas. Em qualquer lugar do mapa.

1. É proibido a realização de loot das forças policiais; Looteamento de policiais só são liberados em ações de Lojinhas, invasões de favelas e ações grandes;
...
1. Ao colocar um suspeito na viatura, deve-se pedir para o meliante colocar o cinto. Caso o mesmo não coloque o sinto e caia mais de uma vez será entendido como anti rp, sujeito a advertência.
...
1. Em uma possível abordagem, deve-se evitar conflito caso esteja em menor número. Combates só serão liberados caso não consiga fugir ou fique encurralado.

### INVASÕES E PACIFICAÇÕES

1. Deverá ser apresentado inquérito policial com mandato judicial para operações em tal escala. A força policial não precisa de aviso prévio para realização de tal ação, desde que amparado pela lei da cidade. Havendo pacificação de uma facção por regra, a facção perderá 50% do loot total e ficará proibida de realizar qualquer atividade durante o prazo de 24 horas (vinte e quatro horas). Tais mandatos têm validade de 24 horas para realização, não sendo cumpridos deverão ser solicitado novo mandato;

1. É necessário autorização prévia do Prefeito para realizar invasão ou pacificação.

1. É necessário ter um motivo válido para invasão de outra facção, sendo que não será necessário haver um número igual de defensores e atacantes, sendo necessário que seja informado no ilegal sobre o aviso da invasão com antecedência de aproximadamente 20 minutos, não podendo a polícia intervir na invasão.

1. Todas as ações acima descritas, DEVEM seguir o RP, não havendo a interação adequada poderá ser passível de anulação da mesma;

### MAPA DA CIDADE

1. O mapa não será mais dividido entre dois polos onde de um lado podemos usar armas pesadas, existem permissões especiais, uma força policial apenas tem autorização para entrar e de outro não. A cidade como um todo é apenas uma,  norte e sul, com os mesmos tipos de privilégios e permissões dos dois lados. Isso fará com que haja mais movimentação no mapa como um todo. Explore o mapa, utilize-o de forma plena, não concentre ações em apenas uma região específica.
